...
def get_long_short(symbol):
    try:
        ...
        if futures_symbol.endswith("USD"):
            futures_symbol = (
                futures_symbol[:-3]
                + "USDT"
            )
...
def send_jet_alert(result):
    ...
    old_alert = last_alerts.get(
        symbol
    )
    # Aynı yönde aynı güçlü sinyal
    # tekrar tekrar gönderilmesin.
    if old_alert == direction:
        return
    if strength >= 95:
        alert_title = (
            "🚀 JET ALERT — EXTREME"
...
def check_new_signals(results):
    for result in results:
        ...
        # Güç 85 altına düşerse
        # yeniden yükseldiğinde alarm
        # verebilmesine izin ver.
        elif (
            result["strength"]
            < 85
        ):
...
def send_report():
    ...
    # Normal rapor gönderilmez.
    # Sadece JET ALERT gönderilir.
    check_new_signals(
        results
    )
    print(
        "✅ Tarama tamamlandı. "
        "Normal rapor gönderilmedi."
    )
# =========================================================
# PERFORMANS
# =========================================================
